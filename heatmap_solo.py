...
def create_score(list_label, list_max, true_positives, false_positives, false_negatives):
    i = 0
    ...
    print('Epoch: 39\t Precision: {}\t Recall: {}'.format(prec, rec))
    
    return prec, rec


def plot_label_max_coordinates(original_image, list_label, list_max, date_time):
    fig, ax = plt.subplots(figsize=(20,10))
    ax.imshow(original_image, cmap='gray')
    ax.scatter(list_label[:,0], list_label[:,1], marker='+')
    ax.scatter(list_max[:,0], list_max[:,1], marker='.')
    ax.axis('off')
    plt.savefig('../Heatmap/label_max_coordinates_{}_solo'.format(date_time.strftime('%d-%m-%y_%H:%M')), bbox_inches='tight')
    plt.close(fig)


def plot_transparent_heatmap_overlay(original_image, output_greater, list_label, list_max, date_time):
    ...
    plt.savefig('../Heatmap/heatmap_overlay_{}_solo'.format(date_time.strftime('%d-%m-%y_%H:%M')), bbox_inches='tight')
    plt.close(fig)


def plot_label_max_coordinates_with_transparent_heatmap_overlay(original_image, output_greater, list_label, list_max, date_time):
    plt.register_cmap(name='VIRIDIS', data=my_viridis)
    cmap = plt.get_cmap('VIRIDIS')
    ...
    plt.savefig('../Heatmap/coordinates_heatmap_overlay_{}_solo'.format(date_time.strftime('%d-%m-%y_%H:%M')), bbox_inches='tight')
    plt.close(fig)


def plot_heatmap_maxfilter_mask_overlay(image_max, output_greater, mask, net_coord, date_time):
    fig, ax = plt.subplots(figsize=(20,10))
    ...
    plt.savefig('../Heatmap/heatmap_maxfilter_mask_overlay_{}_solo'.format(date_time.strftime('%d-%m-%y_%H:%M')), bbox_inches='tight')
    plt.close(fig)


def plot_coordinates_next_to_mask_overlay(original_image, list_label, list_max, image_max, output_greater, mask, net_coord, date_time):
    #TODO
    print('function not yet executable!')


def create_heatmaps(net, true_positives, false_positives, false_negatives, date_time):
    ...
    plot_label_max_coordinates(original_image, list_label, list_max, date_time)

    # heatmap above original image
    plot_transparent_heatmap_overlay(original_image, output_greater, list_label, list_max, date_time)

    # list_max and list_label coordinates above heatmap
    plot_label_max_coordinates_with_transparent_heatmap_overlay(original_image, output_greater, list_label, list_max, date_time)

    # heatmap with maxfilter and mask (heatmap == maxfilter)
    plot_heatmap_maxfilter_mask_overlay(image_max, output_greater, mask, net_coord, date_time)

    # heatmap with maxfilter and mask next to coordinates
    plot_coordinates_next_to_mask_overlay(original_image, list_label, list_max, image_max, output_greater, mask, net_coord, date_time)

    prec, rec = create_score(list_label, list_max, true_positives, false_positives, false_negatives)

    return prec, rec

def loop_epoches(net, date_time):
    print('*** Create heatmaps using state of chosen epoch ***')
    true_positives = 0.0
    false_positives = 0.0
    false_negatives = 0.0

    net.load_state_dict(torch.load('../Training/training_epoch-39'))
    prec, rec = create_heatmaps(net, true_positives, false_positives, false_negatives, date_time)

    return prec, rec


def heatmap_solo(net, date_time):
    prec, rec = loop_epoches(net, date_time)
    return prec, rec